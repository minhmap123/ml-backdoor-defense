...
class MMBDDetector(BaseDetector):
    ...
    def _assert_numeric_only_context(self, context: DetectorContext) -> None:
        ...
        if isinstance(context.detection_split, dict):
            assert context.detection_split.get("x_cat") is None
...
class MMBDCSODetector(MMBDDetector):
    ...
    - We implement MMBD-CSO directly from the published equations because no
      ...
      the local numeric IDS adaptation, while the CSO term penalizes positive
      cosine similarity between optimized candidate features and the target class
      intrinsic feature subspace learned from clean support data.
    """
