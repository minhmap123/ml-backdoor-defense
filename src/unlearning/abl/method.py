...
class ABLUnlearner(BaseUnlearner):
    ...
    - stage 2: optionally fine-tune on the non-isolated pool, then run gradient
      ascent on the isolated subset to suppress backdoor behavior.
    ...
    - stage-2 repair initializes from the in-memory attacked benchmark model by
      default so the method plugs into the shared runtime contract cleanly.
    """
    ...
    def __init__(self, cfg: Any) -> None:
        ...
        self.stage2_init = str(self.resolved_cfg.get("stage2_init", "attacked_model")).lower()
    ...
    def _run_impl(self, context: UnlearningContext, forget_set: ForgetSet) -> UnlearningResult:
        ...
        if self.save_isolation_checkpoint:
            ...
            artifacts.extra_files["isolation_checkpoint_dir"] = str(isolation_checkpoint_dir)
        ...
        if self.save_checkpoint:
            checkpoint_dir = save_model(
                ...
                metadata={
                    ...
                    "uses_local_tabular_runtime": True,
                },
                optimizer=unlearning_optimizer,
                metrics={
                    "abl/num_isolated": float(len(isolated_indices)),
        ...
        deviation_note = (
            ...
            f"and stage-2 repair initializes from '{self.stage2_init}' for integration with the shared runtime."
        )
    ...
    def _run_epoch(
        ...
        *,
        ...
        return {
            "ce_loss": float(total_ce_loss / divisor),
            "objective_loss": float(total_objective_loss / divisor),
            "accuracy": float(total_correct / divisor),
        }
        ...
        *,
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                losses.append(batch_losses.detach().cpu().numpy().astype(np.float32, copy=False))
        if not losses:
    ...
    def _select_isolated_indices(self, loss_ranking: np.ndarray, *, num_train: int) -> np.ndarray:
        if num_train <= 1:
            return np.asarray(loss_ranking[:1], dtype=np.int64)

        requested = int(np.floor(float(num_train) * float(self.isolation_ratio)))
        if requested <= 0:
            requested = 1
        requested = min(requested, num_train - 1)
        return np.asarray(loss_ranking[:requested], dtype=np.int64)
