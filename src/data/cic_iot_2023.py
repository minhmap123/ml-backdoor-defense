...
# Raw sub-attack labels grouped into 6 attack families.
# We drop the two heterogeneous / very low-support families ("Web", "BruteForce")
# below; see LABELS_TO_DROP rationale.
GROUPS = {
    "DDoS": [
...
# Dropped families. Decision rationale:
#   - The original CICIoT2023 paper (Neto et al. 2023, Sensors) itself reports
#     Random Forest 8-class F1 = 0.719, DNN F1 = 0.697, and explicitly notes
#     "web-based attacks are usually classified as benign, Recon, or spoofing"
#     and "brute force classification" faces the same issue.
#     https://pmc.ncbi.nlm.nih.gov/articles/PMC10346235/
#   - "Web" merges 6 heterogeneous app-layer attacks (XSS, SQLi, BACKDOOR_MALWARE,
#     etc.); flow-level aggregated features (rate, IAT, packet sizes) cannot
#     distinguish them from benign HTTP — a feature-engineering ceiling, not a
#     preprocessing artifact.
#   - Our prior all-8-class run reproduced this: BruteForce F1 = 0.09,
#     Web F1 = 0.12 — too unstable for a clean backdoor-defense baseline.
#   - Matches the same drop-minority rationale we use for CIC-IDS2017
#     (cic_ids2017.py: Heartbleed, Infiltration).
LABELS_TO_DROP = {
    "BruteForce",   #  12,522 samples (0.03% of dataset) → F1 < 0.10 unaided
    "Web",          #  23,795 samples (0.05% of dataset) → F1 < 0.12 unaided
}

# Alphabetical to match LabelEncoder default ordering.
CLASS_LIST = ["Benign", "DDoS", "DoS", "Mirai", "Recon", "Spoofing"]

# DDoS/DoS/Mirai alone are ~85% of all flows; cap each in train only to prevent
# bias toward flood traffic. Val/test keep natural distribution so reported
# accuracy reflects deployment-time class proportions (CSE-CIC-IDS2018 pattern).
TRAIN_CAPS = {
    "DoS":   1_500_000,
    "DDoS":  1_500_000,
    "Mirai": 1_500_000,
...
class CICIoT2023Dataset(NumericIDSDataset):
    SCALER_TYPE = "QuantileTransformer->MinMaxScaler"

    schema = DatasetSchema(
        name="CIC-IoT2023",
        raw_path=Path("data/0_raw/CIC_IoT_2023"),
        target_column="Label",
        # CICIoT2023 features are aggregated per-flow stats (Header_Length,
        # Protocol Type, Rate, Tot sum/Min/Max/AVG/Std, IAT, ...) — no
        # session identifiers (no Flow ID / IPs / Ports / Timestamp) leak
        # in the raw CSVs, so nothing to drop here.
        drop_columns=(),
        test_size=0.15,
    ...
    def _load_all(self) -> pd.DataFrame:
        csvs = sorted(Path(self.schema.raw_path).glob("Merged*.csv"))
        return pd.concat(
    ...
    def _select_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Step 2: duplicate-value columns (hash + equals confirm)
        seen, drop2 = {}, []
    ...
    def prepare_clean_partitions(self) -> dict:
        ...
        # Preserve raw sub-attack labels (stratified train-cap below uses these
        # so the cap keeps proportional representation of each sub-attack).
        target = self.schema.target_column
        raw_labels = df[target].astype(str).str.strip()
        mapping = {label: cls for cls, labels in GROUPS.items() for label in labels}
        df[target] = raw_labels.replace(mapping)

        mask = df[target].isin(LABELS_TO_DROP)
        df = df.loc[~mask].reset_index(drop=True)
        raw_labels = raw_labels.loc[~mask].reset_index(drop=True)

        X_raw = self._select_features(df.drop(columns=[target]))
        y = df[target]
        ...
        # Cap DoS/DDoS/Mirai only in train, stratified by raw sub-attack labels
        # so each underlying attack variant is sampled proportionally.
