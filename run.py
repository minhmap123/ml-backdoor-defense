...
def main(cfg: DictConfig) -> None:
    ...
    stage = str(cfg.get("pipeline", {}).get("stage", "full")).strip().lower()
    if stage in {"attack_train", "attack-train"}:
        _run_stage_or_record_failure(cfg, "attack_train", run_attack_train_stage)
        return
    if stage == "detection":
        _run_stage_or_record_failure(cfg, "detection", run_detection_stage)
        return
    if stage == "unlearning":
        _run_stage_or_record_failure(cfg, "unlearning", run_unlearning_stage)
        return
    if stage != "full":
        raise ValueError(f"Unknown pipeline.stage={stage!r}. Expected full, attack_train, detection, or unlearning.")
